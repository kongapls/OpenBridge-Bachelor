...
function connectModbus() {
  client
    .connectTCP("158.38.140.60", { port: 502 })
    .then(() => {
...
function sendData() {
  ...
  setInterval(() => {
    if (i < NUMSAMPLES) {
      value =
        Math.sin((2 * Math.PI * FREQUENCY * i) / NUMSAMPLES) * AMPLITUDE +
        OFFSET;
      i++;
      ...
      client
        .writeRegisters(REGISTERADDRESS, [value]) //send values
        .catch((error) => {
          console.error(error);
        });
      if (DEBUG) {
...
function startReadingModbus() {
  setInterval(() => {
    client.readInputRegisters(0, 7, function (err, data) {
      ...
      if (DEBUG) {
        console.log(
          "IN: Sine Scaled Big data: " + SINSCALED,
          "Sine Raw data: " + SINVALUE,
          "Knapp 2 Verdi: " + data.data[SWITCHTWO],
          "Knapp 1 Verdi: " + data.data[SWITCHONE],
          "Modbus data: " + data.data[RANDOMCOUNT]
        );
      }
...
ipcMain.on("get-modbus-data", (event) => {
  client.readInputRegisters(0, 7, function (err, data) {
    ...
    if (DEBUG) {
      console.log(
        "PUSHED: Sine Scaled Big data: " + SINSCALED,
        "Sine Raw data: " + SINVALUE,
        "Knapp 2 Verdi: " + data.data[SWITCHTWO],
        "Knapp 1 Verdi: " + data.data[SWITCHONE],
        "Modbus data: " + data.data[RANDOMCOUNT]
